...
import { getRecipesPrompt } from '../data/coachResponses'
  ...
  return (
    <div className="px-4 py-5 pb-28 flex flex-col gap-4">
      <div className="fade-up flex items-start justify-between">
        ...
        <button
          onClick={handleCopyRecipesPrompt}
          className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center tap-scale transition-colors"
          style={copied ? { background: 'rgba(74, 222, 128, 0.1)' } : {}}
          title="Copier le prompt recettes"
        >
          {copied ? <Check size={18} className="text-green-400" /> : <Sparkles size={18} className="text-white/50" />}
        </button>
      </div>
